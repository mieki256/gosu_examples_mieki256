...
# Last updated: <2019/03/15 02:50:51 +0900>
#
# wavefront(.obj) read, parse and dump
#
# usage :
#   ruby wavefrontobj.rb INFILE.obj [--no-index] [--no-index] [--debug]
#
...
class WaveFrontObj
  ...
  def make_varray_data
    if @vertex_array_data.empty?
      if @use_index
        make_varray_with_index
      else
        make_varray_not_with_index
      end
    end
  end

  def make_varray_not_with_index
    vertexs = []
    normals = []
    uvs = []
...
if $0 == __FILE__
  ...
  require "optparse"
  opts = {:varray => true, :index => true, :debug => false}
  ...
  o = WaveFrontObj.new(opts[:infile], opts[:varray], opts[:index])
