...
# Last updated: <2019/03/18 21:32:07 +0900>
#
...
class GlObj
  ...
  def initialize(pos_x = 0.0, pos_y = 0.0, pos_z = -3.0)
    ...
    if USE_MY_SHADER
      SHADER_SRC_LIST.each do |vert_src_fn, frag_src_fn|
        @shaders.push(init_shader(vert_src_fn, frag_src_fn))
      end
    end
  ...
  def exec_gl
    ...
    if USE_MY_SHADER
      if @uv
        glUseProgram(@shaders[1])  # テクスチャ使用シェーダ
      else
        glUseProgram(@shaders[0])  # テクスチャ未使用シェーダ
      end
    end
