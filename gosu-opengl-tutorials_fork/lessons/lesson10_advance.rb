...
class TjTexture
  attr_accessor :info, :image, :width, :height

  def initialize(window, name)
    @image = Gosu::Image.new("res/tj/lesson10_advance/#{name}.jpg", :tileable => true)
    @width = @image.width
    @height = @image.height
    @info = @image.gl_tex_info
    abort "Error : #{name}.jpg is not load. Can't get gl_tex_info" unless @info
    return @info
  end
end
...
class Window < Gosu::Window
  ...
  def setup_world
    world = JSON.parse(File.read('res/tj/lesson10_advance/world.json'))
    ...
    @all_meshes.keys.each do |name|
      ...
      unless $glbind
        ...
        gluBuild2DMipmaps(GL_TEXTURE_2D, 3,
                          minimap.image.width, minimap.image.height,
