...
class Texture
  ...
  def initialize(window)
    @image = Gosu::Image.new("res/nehe/lesson09/Star.bmp", :tileable => true)
    @width = @image.width
    @height = @image.height
  ...
  def draw
    gl do
      ...
      @stars.each_with_index do |star, i|
        ...
        # since we are using a flat texture
        # we want it always look flat and correct regardless
        # how much spinning and rotation are we doing.
        # We can archive that by canceling the rotations
        # that we are doing before we draw our stars.
        # IMPORTANT: Cancellation by happen in the reverse order
        glRotatef(-star.angle, 0, 1, 0)
        glRotatef(-@tilt, 1, 0, 0)

        # we make star swinkle by drawing another random non spinning star
        # on the bottom of it
        draw_star(@stars[rand(@stars.size)]) if @twinkle_on
        ...
        # if it reaches center, restart its distance and reset the color
        if (star.distance < 0)
          star.distance += 5
          star.color_randomly
        end
